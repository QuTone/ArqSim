...
The objects in this module are advanced extension contracts.  Components
...
RUNTIME_COMPONENT_SCHEMA_VERSION = "arqsim.runtime-component.v4"
RUNTIME_MANIFEST_SCHEMA_VERSION = "arqsim.runtime-manifest.v4"
...
def default_event_engine_descriptor() -> RuntimeComponentDescriptor:
    return RuntimeComponentDescriptor(
        role="event_engine",
        component_id="event_engine.transactional_dynamic_frontier.v3",
        effective_config={
            "timestamp_phases": [
                "complete_all",
                "program_plane_first",
            ...
            "measurement_dispatch": "expected_logical_measurements_only",
            "measurement_seed_stream": "logical_measurement.v1",
        },
    )
...
def default_runtime_component_manifest() -> RuntimeComponentManifest:
    ...
    return RuntimeComponentManifest(
        components={
            ...
            "measurement_provider": RuntimeComponentDescriptor(
                "measurement_provider",
                "measurement.seeded_bernoulli.v1",
                effective_config={
                    "distribution": "independent_bernoulli_half",
                    "seed_identity": "program_work_lineage_and_register",
                },
            ),
        }
    )
...
@dataclass(frozen=True)
class LogicalMeasurementRequest:
    """Immutable context for one event that must produce logical bits."""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.snapshot, StateSnapshot):
            raise RuntimeComponentError(
                "Logical measurement snapshot must be a StateSnapshot"
            )
        if not isinstance(self.operation, RuntimeOperationView):
            raise RuntimeComponentError(
                "Logical measurement operation must be a RuntimeOperationView"
            )
        if not isinstance(self.reservation, StateReservation):
            raise RuntimeComponentError(
                "Logical measurement reservation must be a StateReservation"
            )
        if (
            ...
            raise RuntimeComponentError(
                "Logical measurement candidate_id must be a canonical "
                "non-empty string"
            )
        if type(self.event_id) is not int or self.event_id < 0:
            raise RuntimeComponentError(
                "Logical measurement event_id must be a non-negative integer"
            )
        start_s = _exact_nonnegative_float(
            self.start_s,
            label="Logical measurement start time",
        )
        end_s = _exact_nonnegative_float(
            self.end_s,
            label="Logical measurement end time",
        )
        if end_s < start_s:
            raise RuntimeComponentError(
                "Logical measurement end time cannot precede its start time"
            )
        if type(self.measurement_seed) is not int or self.measurement_seed < 0:
            raise RuntimeComponentError(
                "Logical measurement seed must be a non-negative integer"
            )
        measurements = tuple(self.expected_measurements)
        if not measurements or any(
            type(value) is not str or not value for value in measurements
        ):
            raise RuntimeComponentError(
                "LogicalMeasurementRequest expected_measurements must contain "
                "non-empty string ids"
            )
        if len(measurements) != len(set(measurements)):
            raise RuntimeComponentError(
                "LogicalMeasurementRequest repeats a measurement register"
            )
        if self.operation.plane != "program":
            raise RuntimeComponentError(
                "LogicalMeasurementRequest only supports Program gadget work"
            )
        if not isinstance(self.lineage, ProgramWorkLineage):
            raise RuntimeComponentError(
                "Program logical measurement needs typed lineage"
            )
        if self.lineage.step != "measurement":
            raise RuntimeComponentError(
                "LogicalMeasurementRequest requires measurement-step lineage"
            )
...
@dataclass(frozen=True)
class LogicalMeasurementOutcome:
    register_id: str
    bit: int
...
@dataclass(frozen=True)
class LogicalMeasurementResult:
    """Logical measurement bits returned by a measurement provider."""

    measurements: tuple[LogicalMeasurementOutcome, ...] = ()

    def __post_init__(self) -> None:
        measurements = tuple(self.measurements)
        if any(
            not isinstance(item, LogicalMeasurementOutcome)
            for item in measurements
        ):
            raise RuntimeComponentError(
                "LogicalMeasurementResult measurements must be typed "
                "LogicalMeasurementOutcome values"
            )
        register_ids = [item.register_id for item in measurements]
        if len(register_ids) != len(set(register_ids)):
            raise RuntimeComponentError(
                "LogicalMeasurementResult repeats a measurement register"
            )
    ...
    def bit_for(self, register_id: str) -> int:
        ...
        if len(matches) != 1:
            raise RuntimeComponentError(
                "LogicalMeasurementResult does not contain exactly one bit "
                f"for {register_id!r}"
            )
        return matches[0]
...
class ExecutionTimingBackend(Protocol):
    descriptor: RuntimeComponentDescriptor

    def prepare(self, request: BackendRequest) -> PreparedExecution: ...


class LogicalMeasurementProvider(Protocol):
    descriptor: RuntimeComponentDescriptor
...
def validate_builtin_component_source_context(
    ...
    *,
    ...
    if component.descriptor.component_id != "runtime_realizer.state_bound.v3":
        return
...
@dataclass(frozen=True)
class _SeededMeasurementProvider:
    descriptor: RuntimeComponentDescriptor
...
class RuntimeComponentSet:
    ...
    execution_backend: ExecutionTimingBackend
    measurement_provider: LogicalMeasurementProvider
...
def build_runtime_component_set(
    ...
    *,
    ...
    return RuntimeComponentSet(
        ...
        execution_backend=_DefaultTimingBackend(
            manifest.components["execution_backend"]
        ),
        measurement_provider=_SeededMeasurementProvider(
            manifest.components["measurement_provider"]
        ),
    )
