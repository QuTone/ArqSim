...
from arqsim.architecture.recipes import ProgramWorkLineage
...
EXECUTION_TRACE_SCHEMA_VERSION = "arqsim.execution-trace.v4"
...
class ExecutionTrace:
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "ExecutionTrace":
        ...
        if not _exact_json_equal(unsigned, result.semantic_dict()):
            raise TraceValidationError(
                "ExecutionTrace input is not the exact canonical v4 wire form"
            )
...
class EvaluationResult:
    ...
    def diagnostic_dict(self) -> dict[str, Any]:
        """Return a non-wire diagnostic projection of this live result.

        This mapping is intentionally not versioned and is not accepted by
        :class:`ExecutionTrace`. Persist ``trace.to_dict()`` for the canonical
        runtime artifact, or an ``EvaluationReport`` for the complete public
        result. The flattened projection exists only for internal diagnostics
        and the frozen Report-v1 compatibility renderer.
        """
...
def replay_execution_trace(trace: ExecutionTrace, plan: Any) -> TraceStateProjection:
    ...
    from arqsim.architecture.state import ArchitectureState, TentativeBinding
    ...
    execution_backend_id = str(
        plan.runtime_components["components"]["execution_backend"][
            "component_id"
        ]
    )
    resource_by_id = {
    ...
    def measurement_work_id(source_id: int, members: tuple[Any, ...]) -> str:
        if len(members) == 1:
            member = members[0]
            return continuation_work_id(
                source_id,
                recipe_by_invocation[member.recipe_invocation_id],
                member.stage_index,
                "measurement",
            )
        ...
        if len(stages) != 1:
            raise TraceReplayError(
                "Shared measurement lineage must use one common stage"
            )
        return f"program:{source_id}:stage:{next(iter(stages))}:shared-measurement"
    ...
    def validate_program_continuation(
        ...
        if lineage.parent_event_id is None:
            ...
            if transition.measurements:
                raise TraceReplayError(
                    "Static source/entangle completion cannot emit measurements"
                )
            if not recipes:
                if lineage.step != "source" or lineage.recipe_members:
                    raise TraceReplayError(
                        "Recipe-free Program source has invalid lineage"
                    )
                expected = ProgramContinuationReceipt("complete_source")
            else:
                expected_members = tuple(
                    (recipe.invocation_id, 0) for recipe in recipes
                )
                actual_members = tuple(
                    member.key for member in lineage.recipe_members
                )
                if (
                    lineage.step != "entangle"
                    or actual_members != expected_members
                ):
                    raise TraceReplayError(
                        "Entangle source members do not match frozen recipes"
                    )
                ...
                expected = ProgramContinuationReceipt(
                    "activate",
                    (measurement_work_id(source_id, lineage.recipe_members),),
                )
        elif lineage.step == "measurement":
            expected_registers: set[str] = set()
            for member in lineage.recipe_members:
                recipe = recipe_by_invocation.get(member.recipe_invocation_id)
                if (
                    recipe is None
                    or member.stage_index >= len(recipe.stages)
                    or recipe not in getattr(source, "implementation_recipes", ())
                ):
                    raise TraceReplayError(
                        "Measurement completion has an unknown recipe member"
                    )
                expected_registers.add(
                    recipe.measurement_register(member.stage_index)
                )
            if set(transition.measurements) != expected_registers:
                raise TraceReplayError(
                    "Measurement completion registers do not match its members"
                )
            expected = ProgramContinuationReceipt(
                "activate",
                tuple(
            ...
            member = lineage.recipe_member
            recipe = (
                recipe_by_invocation.get(member.recipe_invocation_id)
                if member is not None
                else None
            )
            if recipe is None or member is None:
            ...
            if lineage.step == "injection":
                if transition.measurements:
                    raise TraceReplayError(
                        "Injection entangle completion cannot emit a measurement"
                    )
                expected = ProgramContinuationReceipt(
                    "activate",
                    (measurement_work_id(source_id, lineage.recipe_members),),
                ...
                if (
                    ...
                    or not any(
                        parent_member.recipe_invocation_id
                        == recipe.invocation_id
                        and parent_member.stage_index == stage_index
                        for parent_member in parent.program_lineage.recipe_members
                    )
        ...
        if transition.kind == ExecutionTransitionKind.DISPATCH:
            if transition.plane == ExecutionPlane.PROGRAM:
                ...
                if lineage.parent_event_id is None:
                    ...
                    expected_duration = (
                        source.duration_s
                        if source.deferred_dispatch is None
                        else None
                    )
                    expected_step = (
                        "entangle"
                        if source.implementation_recipes
                        else "source"
                    )
                    expected_members = tuple(
                        (recipe.invocation_id, 0)
                        for recipe in source.implementation_recipes
                    )
                    if (
                        lineage.step != expected_step
                        or tuple(
                            member.key for member in lineage.recipe_members
                        )
                        != expected_members
                        ...
                        raise TraceReplayError(
                            "Static Program lineage does not match its source recipes"
                        )
                    ready_s = max(
                        (
                    ...
                    for member in lineage.recipe_members:
                        member_recipe = recipe_by_invocation.get(
                            member.recipe_invocation_id
                        )
                        if (
                            member_recipe is None
                            or member_recipe not in getattr(
                                source, "implementation_recipes", ()
                            )
                            or member.stage_index >= len(member_recipe.stages)
                        ):
                            raise TraceReplayError(
                                "Unknown continuation recipe member for "
                                f"{lineage.work_id!r}"
                            )
                    expected_work_id = (
                        measurement_work_id(
                        ...
                        else continuation_work_id(
                            instruction_id,
                            recipe_by_invocation[
                                lineage.recipe_members[0].recipe_invocation_id
                            ],
                            lineage.recipe_members[0].stage_index,
                            lineage.step,
                        )
                    )
                    if lineage.work_id != expected_work_id:
                        raise TraceReplayError(
                            f"Unknown continuation identity {lineage.work_id!r}"
                        )
                    templates = tuple(
                        template
                    ...
                    if len(templates) != 1:
                        raise TraceReplayError(
                            f"Continuation {lineage.work_id!r} has no unique "
                            "frozen Program-work template"
                        )
                    template = templates[0]
                    expected_opcode = template.opcode
                    expected_consumes = template.consumes
                    expected_produces = template.produces
                    expected_forwards = template.forwards
                    expected_engines = template.engines
                    expected_required = template.required_locations
                    expected_completion = template.completion_locations
                    expected_qubits = template.qubits
                    expected_targets = template.target_modules
                    expected_links = template.target_links
                    expected_metadata = dict(template.metadata)
                    expected_duration = template.duration_s
                    ready_s = next(
                        item.end_s
                    ...
                    if lineage.step not in {
                        "reaction",
                        "injection",
                        "measurement",
                        "correction",
                    }:  # ProgramWorkLineage closes this union.
                ...
                if (
                    ...
                    raise TraceReplayError(
                        f"Program work {lineage.work_id!r} duration disagrees "
                        "with its frozen Plan operation"
                    )
...
def validate_execution_trace_document(data: Mapping[str, Any]) -> ExecutionTrace:
    """Validate one exact canonical ExecutionTrace-v4 document.

    The non-wire :meth:`EvaluationResult.diagnostic_dict` projection and
    derived event/final-state projections are deliberately not accepted.
    Report v1 owns its one-way compatibility projection; callers with a Plan
    may additionally use :func:`replay_execution_trace` for full
