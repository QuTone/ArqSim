...
class ResourceProcess(OperationClaims):
    ...
    def __post_init__(self) -> None:
        ...
        if not self.protocol.strip():
            raise ValueError("Resource process protocol must be non-empty")
        ...
        _validate_diagnostic_metadata(
            self.metadata,
            label=f"{opcode.value} resource process",
        )
        ...
        if opcode in {
            ...
            if not produces:
                raise ValueError(
                    f"{opcode.value} must declare at least one produced output"
                )
            if forwards:
                raise ValueError(
                    f"{opcode.value} creates resource state and cannot forward "
                    "an existing token"
                )
        if opcode == ArchitectureOpcode.TELEPORT_QUBITS:
            if not self.target_links:
                raise ValueError(
                    "Resource TELEPORT_QUBITS must identify at least one target link"
                )
            if not consumes or not produces or not forwards:
                raise ValueError(
                    "Resource TELEPORT_QUBITS must consume input resources, "
                    "produce an output, and forward the teleported token"
                )
            if not any(
                source != destination
                ...
                raise ValueError(
                    "Resource TELEPORT_QUBITS must move at least one forwarded "
                    "token to a different buffer"
                )
