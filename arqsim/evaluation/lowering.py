...
from arqsim.compiler.models import LogicalCompilerSpec, LogicalLayout
...
from arqsim.compiler.pipeline import (
    DefaultCompilerPipeline,
    build_runtime_route_compiler,
)
from arqsim.compiler.interface import CompilerPipeline
...
PLAN_BUILDER_VERSION = "arqsim-plan-builder-v5"
...
def _lower_architectural_state(
    ...
    *,
    ...
    bell_engine_owners: dict[str, tuple[str, str]] = {}
    for interconnect in specification.interconnects:
        bell_engines = tuple(
            (module, submodule)
            for module in interconnect.modules
        ...
        if len(bell_engines) != 1:
            raise ValueError(
                f"Interconnect {interconnect.id} needs one Bell engine"
            )
        bell_engine_module, bell_engine = bell_engines[0]
        bell_engine_owners[interconnect.id] = (
            absolute_module_id(interconnect.id, bell_engine_module.id),
            absolute_submodule_id(
                interconnect.id,
                bell_engine_module.id,
                bell_engine.id,
            ),
        )
        bell_buffers = tuple(
    ...
    engines.extend(
        EngineSpec(
            ...
            int(profile["copies"]),
            module=bell_engine_owners[link_id][0],
            submodule=bell_engine_owners[link_id][1],
        )
        ...
        engines.extend(
            (
                ...
                EngineSpec(
                    ...
                    topology.store_load_buffer_capacity,
                    module=topology.compute_location,
                    submodule=topology.compute_buffer_location,
                ),
            )
        )
    if msf is not None:
        engines.extend(
            (
                ...
                # Resource delivery concurrency is derived from interface-buffer
                # capacity, not owned by the MSF Module.  Its operation-level
                # semantic locus remains explicit in ResourceProcess.target_links.
                EngineSpec(
                    "resource_move",
...
def lower_compilation_result(
    ...
    *,
    ...
    expected_compilation_sources = {
        "circuit_hash": circuit.semantic_hash,
        "architecture_hash": specification.architecture_hash,
        "latency_profile_hash": latency_profile.binding_hash,
    }
    ...
    if actual_compilation_sources != expected_compilation_sources:
        raise LogicalCompilerValidationError(
            ...
            details={
                "expected_sources": expected_compilation_sources,
                "actual_sources": actual_compilation_sources,
            },
        )
    if (
        compilation.magic_state_consumption
        != execution_policy.magic_state_consumption
    ):
        raise LogicalCompilerValidationError(
            "LogicalCompilationResult magic-state consumption does not match the "
            "execution policy: expected "
            f"{execution_policy.magic_state_consumption!r}, got "
            f"{compilation.magic_state_consumption!r}",
            details={
                "expected_magic_state_consumption": (
                    execution_policy.magic_state_consumption
                ),
                "actual_magic_state_consumption": (
                    compilation.magic_state_consumption
                ),
            },
        )
    ...
    topology = _runtime_topology(specification, layout)
    ...
    cost_inputs = _resolve_plan_cost_inputs(
        specification,
        topology,
        latency_profile,
    )
    ...
    if not execution_units:
        ...
        return ExecutionPlan(
            circuit_hash=circuit.semantic_hash,
            architecture_hash=specification.architecture_hash,
            latency_profile_hash=latency_profile.profile_hash,
    ...
    return ExecutionPlan(
        ...
        provenance={
            ...
            "runtime_injection_recipe_mode": (
                execution_policy.runtime_injection_mode.value
            ),
...
def compile_and_lower(
    ...
    *,
    ...
    from :mod:`arqsim.evaluation`, but deliberately remains outside the
    package-root facade.
    """
    ...
    pipeline = compiler_pipeline or DefaultCompilerPipeline(
        ...
        magic_state_consumption=execution_policy.magic_state_consumption,
        defer_magic_routing=True,
    )
    compilation = pipeline.compile(circuit, specification, latency_profile)
    if not isinstance(compilation, LogicalCompilationResult):
        raise LogicalCompilerValidationError(
            "CompilerPipeline must return a LogicalCompilationResult",
            details={
                "actual_type": type(compilation).__name__,
            },
        )
