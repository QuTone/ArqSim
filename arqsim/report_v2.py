...
evaluation without rerunning the logical compiler, scheduler, measurement provider,
...
This module deliberately does not import :mod:`arqsim.api` at import time.
...
from arqsim.compiler.config import canonical_compiler_spec
from arqsim.compiler.output import LogicalCompilationResult
from arqsim.evaluation.analysis import (
    EvaluationAnalysis,
    analyze_evaluation,
    qubit_exposure,
)
...
from arqsim.evaluation.lowering import lower_compilation_result
from arqsim.evaluation.plan import ExecutionPlan
from arqsim.evaluation.result import (
    EvaluationResult,
    ExecutionPlane,
    ExecutionTrace,
...
if TYPE_CHECKING:
    from arqsim.api import EvaluationConfig
...
def _summary(
    *,
    ...
    return {
        ...
        "invariant_checks": normalize_json(invariant_checks),
    }
...
class ReportV2Codec:
    ...
    def from_dict(cls, document: Mapping[str, Any]) -> ReportV2Document:
        ...
        if config.run_label is not None and config.run_label != workflow_id:
            raise ReportV2ValidationError(
        ...
        expected_architecture = build_architecture_specification(
            ...
            policy_overrides=config.architecture_overrides,
        ...
        if config.footprint_model == "protocol_aware_reference_v1":
            expected_footprint_model = PhysicalFootprintModel.reference_v1()
        else:
            if not isinstance(config.footprint_model, PhysicalFootprintModel):
                raise ReportV2ValidationError(
                    "Unknown requested footprint-model selection"
                )
            expected_footprint_model = config.footprint_model
        ...
        # This validator fully replays the trace against the Plan before
        # checking either summary or full observations.
...
class ReportV2Renderer:
    ...
    def render(
        ...
        *,
        ...
        # The codec has already detached and recursively frozen this document.
        # Public to_dict() still returns an independent mutable JSON projection.
        return parsed._document
