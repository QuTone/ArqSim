...
def parse_gate_qasm(
    ...
    *,
    ...
    """Normalize gate-based OpenQASM 2 and derive conservative DAG layers."""
...
def parse_clifford_t_qasm(
    ...
    *,
    ...
    """Backward-compatible loader for the legacy ``clifford_t`` dialect."""

    return parse_gate_qasm(
        path,
...
def load_ft_workload(
    ...
    *,
    ...
    """Normalize a synthesized artifact into compiler-facing logical IR.

    ``representation`` selects the artifact parser, not a globally supported
    gate set.  ``gate`` is the gate-set-neutral OpenQASM dialect; the legacy
    ``clifford_t`` spelling uses the same parser and remains hash-compatible.
    Compiler backends validate whether they can implement the resulting
    operations.
    """
    ...
    if representation in {"gate", "clifford_t"}:
        return parse_gate_qasm(
            source,
            representation=representation,
            provenance=provenance,
    ...
    raise WorkloadParseError(
        f"No workload parser is registered for representation: {representation}",
        details={
            "representation": representation,
            "available_parsers": ["clifford_t", "gate", "pbc"],
        },
    )
...
def workload_stats(workload: FTCircuit) -> dict[str, Any]:
    ...
    # Count semantic T operations from the operations themselves.  An IR
    # dialect label is not a gate-set proof.
    t_count = op_counts["t"] + op_counts["tdg"] + op_counts["t_pauli"]
