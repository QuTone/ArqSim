...
def _error_document(exc: Exception) -> dict[str, Any]:
    ...
    error = {
        ...
        "message": str(exc),
        "details": normalize_json(details) if isinstance(details, Mapping) else {},
    }
...
def _synthesize(argv: list[str]) -> int:
    from arqsim.program.errors import InputValidationError, SynthesisError
    from arqsim.synthesizer import SynthesisSpec, Synthesizer
    from arqsim.synthesizer.service import DEFAULT_CACHE_DIR

    parser = argparse.ArgumentParser(prog="arqsim synthesize")
    parser.add_argument("source")
    parser.add_argument("--backend", required=True, choices=("nwqec", "precomputed"))
    parser.add_argument(
        "--representation",
        required=True,
        choices=("gate", "clifford_t", "pbc"),
    )
...
def _evaluate(argv: list[str]) -> int:
    ...
    from arqsim.evaluation import ExecutionPolicy
    ...
    parser = argparse.ArgumentParser(prog="arqsim evaluate")
    parser.add_argument("program", help="Serialized FTCircuit JSON or synthesized QASM")
    parser.add_argument(
        "--representation", choices=("gate", "clifford_t", "pbc")
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        ...
        help="override one architecture sizing, layout, or protocol field",
    )
    ...
    parser.add_argument(
        ...
        "--magic-consumption",
        dest="magic_state_batching",
        choices=("bulk_wave", "incremental"),
    )
    parser.add_argument(
        "--observation-level",
        "--trace-level",
        dest="observation_level",
    ...
    fidelity_group.add_argument(
        ...
        help="select the named fidelity model (default: canonical-reference-v1)",
    )
    fidelity_group.add_argument(
        "--no-fidelity",
        action="store_true",
        help="disable fidelity estimation explicitly",
    )
    parser.add_argument("--run-seed", "--seed", dest="run_seed", type=int)
    parser.add_argument(
        "--report-version",
        choices=("v1", "v2"),
        default="v2",
    ...
    try:
        ...
        config_flags = {
            ...
            "--magic-state-batching": args.magic_state_batching is not None,
            "--observation-level": args.observation_level is not None,
            "--max-transitions": args.max_transitions is not None,
            "--fidelity-profile": args.fidelity_profile is not None,
            "--no-fidelity": args.no_fidelity,
            "--run-seed": args.run_seed is not None,
        }
        ...
        if args.config is not None:
            ...
            default_policy = facade_defaults.execution_policy
            policy = ExecutionPolicy(
                magic_state_batching=(
                    args.magic_state_batching
                    or default_policy.magic_state_batching
                ),
                observation_level=(
                    args.observation_level or default_policy.observation_level
                ),
                run_seed=(
                    args.run_seed
                    if args.run_seed is not None
                    else default_policy.run_seed
                ),
                max_transitions=(
                    args.max_transitions
                    if args.max_transitions is not None
                    else default_policy.max_transitions
                ),
            )
            evaluation_config = EvaluationConfig(
                profile_id=args.architecture or facade_defaults.profile_id,
                architecture_overrides=_key_values(args.policy_overrides),
                latency_profile=latency,
                execution_policy=policy,
                fidelity_profile=(
                    None
                    if args.no_fidelity
                    else facade_defaults.fidelity_profile
                ),
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="arqsim")
