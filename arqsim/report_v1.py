...
from arqsim.compiler.models import LayoutSlot, LogicalLayout
...
_RESOLVED_SYSTEM_SCHEMA_VERSION = "arqsim.resolved-ft-system.v1"
_QEC_SCHEMA_VERSION = "arqsim.qec-configuration.v2"
    ...
    *,
    ...
    semantic = {
        ...
        "configuration": {
            ...
            "version": 1,
            "target_profile": f"arqsim.{profile_id}",
        },
        "bindings": bindings,
    }
    return {
...
def _resolved_architecture_receipt(
    ...
    *,
    ...
    semantic = {
        "schema_version": "arqsim.architecture-spec.v2",
        "profile": {
            "id": f"arqsim.{profile.get('id', 'unknown')}",
            "version": 1,
...
def _footprint_v1_receipt(
    ...
    *,
    ...
    result = {
        "schema_version": "arqsim.physical-footprint-estimate.v1",
        "architecture_hash": specification.architecture_hash,
        "qec_hash": qec_hash,
        "system_hash": system_hash,
...
def _report_v1_execution_receipts(
    ...
    for instruction in plan["program_dag"]["instructions"]:
        if instruction.get("implementation_recipes"):
            raise ValueError(
                ...
                "continuations; inspect the canonical Plan v9 / Trace v4 "
                "artifacts instead"
            )
    ...
    # canonical run artifact remains Plan v9 and is the only replay authority.
    plan["schema_version"] = "arqsim.execution-plan.v5"

    evaluation = normalize_json(artifacts.evaluation.diagnostic_dict())
    # ExecutionTrace v4 has one serialized/hash authority: the ordered causal
