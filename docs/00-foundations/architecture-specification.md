...
Level 0 · Role: architecture foundation · Status: current for ArqSim 0.2.0
...
`CircuitStatistics` belongs to `FTCircuit` and the `arqsim.program` package.
...
`magic_output_origin`.
The `(100, 40)` coordinate selected by the historical Profile 1.1 baseline is a
