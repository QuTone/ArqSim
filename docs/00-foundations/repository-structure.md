...
Level 0 · Role: repository foundation · Status: current for ArqSim 0.2.0
...
- evaluation owns staged lowering from compiler/topology/cost inputs into the
  ...
  the compilation hash in plan provenance, and owns strict `ExecutionPlan` v9,
  the four-role runtime composition, shared-phase typed-continuation
  ExecutionTrace v4, and co-execution;
...
The CLI and the HTTP adapter at `server/` are adapters over
...
compiler/pipeline.py
  ...
  -> evaluation/plan.py::ExecutionPlan v9
...
`measurement_provider` roles without moving these files.
`evaluation/plan.py` owns v9 wire completeness: its parser requires the checked
plan hash, exact JSON container/scalar shapes, full canonical runtime manifest,
complete buffer records, and exact logical-qubit MOVE/recipe/template coherence
rather than filling or normalizing wire fields from constructor defaults.
...
v4; report v1 alone serializes its derived completed-event view.
