...
Status: **Report/API-v2 boundary complete**
...
`magic_output_origin`. The legacy ArqSim 1.1 value
...
`runtime_realizer.state_bound.v2` (or
`runtime_realizer.direct_fifo_injection.v2` for a
direct plan), `scheduler.program_first_eager.v1`, `backend.profile.v1`, and
`outcome.seeded_bernoulli.v1`. Former binding/compiler/Resource-resolver roles and request
...
- Execution-plan initial locations map strings to strings. Operation buffer
  ...
  typed operands. The strict v4 wire recursively accepts only exact JSON
...
- Architecture: `ArchitectureProfile`, `ArchitectureSpecification`, profile
  ...
  come from `heteqsys.architecture`; the gallery convenience builder comes
  from `heteqsys.specification`. Deprecated root aliases still denote the same
  objects during the one-release migration window.
- Profiles: `ArrivalDistribution`, `OperationLatencyProfile`, and
  `FidelityProfile`.
- Configuration: `LogicalCompilerSpec`, `BackendSpec`, `EvaluationPolicy`, and
...
- Evaluation facade: `EvaluationConfig`, `EvaluationReport`,
  ...
  constants and `CANONICAL_FIDELITY_PRESET` are imported from `heteqsys.api`.
...
| `ExecutionPlan` v6 | immutable reproducibility envelope | strict exact-JSON `arqsim.execution-plan.v6`; requires a checked plan hash, full exact `arqsim.runtime-manifest.v3`, complete buffer records, an explicit injection mode, and finite typed implementation recipes; recursively freezes and validates typed DAG contents/claims, exact logical-qubit MOVE location sets, recipe resource pools/stage timing/closure, and globally unique invocation identity; carries `compilation_hash` lineage; older plans fail closed |
...
| native `EvaluationReport` v2 | self-contained stable evaluation boundary with exact request, resolved authorities, logical compilation, Plan v6, Trace v3, and checked results | complete; strict parsing re-lowers compilation, replays Trace, and recomputes all derived results without rerunning compiler or runtime |
...
2. Finite staged injection now covers T and exact angle-doubling continuations;
   automatic STAR lowering, arbitrary gadgets, and joint-window expansion are
   still future work.
...
compilation, Plan v6, and Trace v3 authorities. `results` contains only checked
...
Direct evaluation and Report v2 use `arqsim.execution-trace.v3`, whose exact wire/hash
...
spans from Trace-v3 transitions. The canonical v3 trace parser does not accept
...
`heteqsys.des`, old `heteqsys.synthesis`, or `architecture.config`. Do not make
