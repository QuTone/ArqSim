...
1. Add one folder under `heteqsys/architecture/gallery/`, named from the
...
4. Exercise Profile parsing/round-trip, exact sizing/layout coverage, resolved
...
Do not add a new file under `architecture/specification_templates/` or extend
the old combined `ArchitectureLayoutPolicy` for new work. All six private
specification templates and their template-only wrappers were deleted in Phase
3; do not reintroduce that construction path. The canonical cutover also deleted the shared
adapter, combined layout-policy package, resolved containers, and separate QEC
configuration wrapper after the compiler/evaluator cutover.
...
`heteqsys.architecture.gallery.na_mcf.sizing` or
`heteqsys.architecture.gallery.na_mcf.layout`. Do not create another
...
compiler-active coordinate. The `(100, 40)` value used by the legacy ArqSim
1.1 baseline is a compatibility preset, not a canonical physical fact or a
...
The pipeline result is a strict `heteqsys.logical-compilation-result.v1`
...
Use `heteqsys.evaluation.lower_compilation_result()` to validate and lower an
already serialized or externally produced result. Use
`heteqsys.evaluation.compile_and_lower()` when the framework should run a
...
state-bound-realizer manifest, not the direct/no-op manifest. A component
manifest identifies the algorithm; both official built-in callback contexts
must also match the plan authorities.
...
When extending the v6 plan codec, preserve its exact-JSON gate. `from_dict()`
...
## Add an operation or resource model
