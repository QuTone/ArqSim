...
[`arqsim`](https://github.com/QuTone/ArqSim) Python package and exposes a small
...
- Python 3.10+
- The ArqSim monorepo checkout, with the server next to the core and frontend:

  ```
  ArqSim/
  ├── arqsim/               # core engine package
  ├── frontend/             # React UI
  └── server/               # this adapter
  ```

  Bundled normalized Clifford+T and PBC workloads live under `server/benchmark/`.
...
`setup.sh` always runs from the `server/` directory so the editable install
