...
def _parameterized_operation_log_success(
    ...
    qubits = transition.metadata.get("qubits", ())
    multiplicity = len(qubits) if isinstance(qubits, (list, tuple)) else 0
    if multiplicity <= 0:
        multiplicity = int(transition.metadata.get("amount", 0))
    if multiplicity <= 0 and kind == "independent_channels_per_teleported_item":
        multiplicity = sum(
            len(tokens)
            for buffer_name, tokens in transition.consumed_tokens.items()
            if str(buffer_name).startswith("bell:")
        )
    if multiplicity <= 0:
        raise ValueError(
            "Parameterized operation fidelity requires a non-empty item payload"
        )
...
def estimate_fidelity(
    ...
    *,
    ...
    Resource-plane operation carries a fidelity channel; trace buffer names are
    never parsed as a substitute for the plan's typed buffer inventory,
    protocol authority, or application-relevance reachability.
    """
