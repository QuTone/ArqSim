...
INJECTION_RECIPE_SCHEMA_VERSION = "arqsim.injection-recipe.v1"
...
class InjectionStage:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            "attempt_duration_s",
            _finite_nonnegative(
                self.attempt_duration_s,
                label="InjectionStage attempt_duration_s",
            ),
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {
            ...
            "attempt_duration_s": self.attempt_duration_s,
        }
        if self.failure_next_stage is not None:
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "InjectionStage":
        _strict_fields(
            data,
            required={"index", "resource", "attempt_duration_s"},
        ...
        return cls(
            ...
            attempt_duration_s=data["attempt_duration_s"],
...
class InjectionRecipe:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "reaction_duration_s",
            _finite_nonnegative(self.reaction_duration_s, label="reaction_duration_s"),
        )
        object.__setattr__(
            self,
            "correction_duration_s",
            _finite_nonnegative(self.correction_duration_s, label="correction_duration_s"),
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "correction_duration_s": self.correction_duration_s,
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "InjectionRecipe":
        ...
        return cls(
            ...
            reaction_duration_s=data["reaction_duration_s"],
            correction_duration_s=data["correction_duration_s"],
...
def build_angle_doubling_recipe(
    *,
    ...
    typed_durations = tuple(attempt_durations_s)
    ...
    if len(typed_durations) != len(typed_resources):
        raise ValueError("Each angle resource requires one injection-attempt duration")
    ...
    stages = tuple(
        InjectionStage(
            index=index,
            resource=resource,
            attempt_duration_s=typed_durations[index],
            failure_next_stage=(
    ...
    return InjectionRecipe(
        ...
        reaction_duration_s=reaction_duration_s,
        correction_duration_s=correction_duration_s,
    )


@dataclass(frozen=True, slots=True)
class ProgramWorkLineage:
    """Typed causal identity for static and continuation Program work."""

    work_id: str
    source_instruction_id: int
    parent_event_id: int | None = None
    recipe_invocation_id: str | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.step not in {"source", "injection", "reaction", "correction"}:
        ...
        if self.step == "source":
            if self.parent_event_id is not None or any(
                value is not None for value in recipe_fields
            ):
                raise ValueError(
                    "Source work cannot claim a parent event or recipe stage"
                )
        else:
            if self.parent_event_id is None or any(
                value is None for value in recipe_fields
            ):
                raise ValueError("Continuation work requires complete recipe lineage")
            object.__setattr__(
                self,
                "recipe_invocation_id",
                _plain_string(self.recipe_invocation_id, label="recipe_invocation_id"),
            )
            object.__setattr__(
                self,
                "recipe_id",
                _plain_string(self.recipe_id, label="recipe_id"),
            )
            object.__setattr__(
                self,
                "stage_index",
