...
from .recipes import InjectionRecipe
...
class ArchitectureInstruction(OperationClaims):
    ...
    def __post_init__(self) -> None:
        ...
        if implementation_recipes:
            ...
            if any(item.source_layer_index != self.layer_index for item in implementation_recipes):
                raise ValueError(
                    "Injection recipes must refer to their instruction source layer"
                )
            if any(not set(item.qubits) <= set(self.qubits) for item in implementation_recipes):
                raise ValueError(
